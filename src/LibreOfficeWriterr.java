...
public class LibreOfficeWriterr {
    ...
    public static void main(String[] args) {
        ...
        WebElement dataTable = wait.until(ExpectedConditions.presenceOfElementLocated(
                By.xpath("//*[@id=\"app\"]/div/div[3]/div[2]/div/div[3]/table")));
        ...
        for (WebElement row : rows) {
            ...
            // If there are header cells, use them as headers in the Excel sheet
            if (!headerCells.isEmpty()) {
                int cellNum = 0;
                for (WebElement headerCell : headerCells) {
                    Cell excelCell = excelRow.createCell(cellNum++);
                    excelCell.setCellValue(headerCell.getText());
                }
            } else {
                int cellNum = 0;
