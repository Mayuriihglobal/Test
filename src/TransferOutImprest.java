...
public class TransferOutImprest {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		ExtentSparkReporter spark = new ExtentSparkReporter("TransferOut-imprest.html");
...
//---
		...
		WebElement externalFacilityOption = wait.until(ExpectedConditions
				.elementToBeClickable(By.xpath("//p[normalize-space()='External Facility- External facility']")));
		...
		WebElement transfersButton = wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath("//p[normalize-space()='Transfers']")));
		transfersButton.click();

		// Get text from the specified element
		WebElement targetElement = wait
				.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//tbody[1]/tr[1]/td[8]/p[1]")));
		...
		WebElement targetElement1 = wait.until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//td[normalize-space()='amoxicillin 500 mg capsule']")));
		targetElement1.getText();

		extent.createTest("Open the new transfer row").assignCategory("regression testing").assignDevice("Chrome")
				.log(Status.PASS, "data is correct: " + targetElement1.getText());

		// Click on the "Complete" button
		WebElement completeButton = wait.until(ExpectedConditions
				.elementToBeClickable(By.xpath("//button[@class='transfer-action-button complete-button']")));
		completeButton.click();

		extent.createTest("Click the Complete Transfer Button: Approve Entry modal is displayed")
				.assignCategory("regression testing").assignDevice("Chrome")
		...
		extent.createTest("Add a note and click submit").assignCategory("regression testing").assignDevice("Chrome")
				.log(Status.PASS, "Add a note and click submit" + noteBox.getText());
		...
		extent.createTest("double signature check modal pops up apper").assignCategory("regression testing")
				.assignDevice("Chrome").log(Status.PASS, "double signature check modal pops up apper");

		// Click on the sign button
		WebElement signButton = wait.until(ExpectedConditions
				.elementToBeClickable(By.xpath("//div[@class='form-section-container']//div//div[1]//div[1]")));
		signButton.click();
		...
		WebElement signInButton = wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[@class='modal-body']//div[2]//div[1]")));
		signInButton.click();
